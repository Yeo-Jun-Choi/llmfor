...
class MInterface(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        mask = (shift_labels == 13)
        indices = mask.float() * torch.arange(shift_labels.size(1), device=shift_labels.device).unsqueeze(0)
        token_indices = indices.max(dim=1)[1]
    
        # Create masks for before and after the target token
        batch_size, seq_len, vocab_size = shift_logits.size()
        arange = torch.arange(seq_len, device=shift_labels.device).unsqueeze(0)
        before_mask = arange <= token_indices.unsqueeze(1)
        after_mask = arange > token_indices.unsqueeze(1)

        # Apply the masks to get before and after logits and labels
        before_logits = shift_logits[before_mask].view(-1, vocab_size)
        before_labels = shift_labels[before_mask].view(-1)
        after_logits = shift_logits[after_mask].view(-1, vocab_size)
        after_labels = shift_labels[after_mask].view(-1)

        # Calculate the loss for each part individually
        ...
        if batch['flag']:
            ...
            # Easy
            loss = before_loss + (after_loss*self.hparams.reverse_weight)

        self.log('loss', loss, on_step=True, on_epoch=True, prog_bar=True)
        self.log('before_loss', before_loss, on_step=True, on_epoch=True, prog_bar=True)
        self.log('after_loss', after_loss, on_step=True, on_epoch=True, prog_bar=True)
    ...
    def on_validation_epoch_start(self):
        ...
        self.val_content_ = {"generate": [], "real": [], "cans": []}
    ...
    def on_validation_epoch_end(self):
        ...
        df.to_csv(op.join(self.hparams.output_dir, f'valid_callback_{self.current_epoch}.csv'))
    ...
    def load_llm(self, llm_path):
        ...
        self.llama_tokenizer.add_special_tokens({'additional_special_tokens': ['[PH]','[HistoryEmb]','[CansEmb]','[ItemEmb]', '[UserEmb]']})
    ...
    def encode_users(self, batch):
        ...
        if self.hparams.rec_embed=="SASRec":
            item_rec_embs=self.rec_model.cacu_x(batch['seq'])
        elif self.hparams.rec_embed in ['Caser','GRU']:
            item_rec_embs=self.rec_model.item_embeddings(batch['seq'])
        
        weight_list = []
        for i in range(len(batch['seq'])):
            weights = torch.zeros(batch['seq'][0].size())
            num_items = batch['len_seq'][i]
            weight = torch.linspace(0.1, 1.0, steps=num_items)
            weight = weight / weight.sum()
            weights[:num_items] = weight
            weight_list.append(weights)
        weight_list = torch.stack(weight_list).unsqueeze(-1).to(item_rec_embs.device)
        user_embedding = (weight_list * item_rec_embs).sum(dim=1)
        
        user_txt_embs = self.projector(user_embedding)
    ...
    def wrap_emb(self, batch):
        ...
        user_embeds = self.encode_users(batch)
        ...
        for i in range(len(batch["len_seq"])):
            ...
            if (batch["tokens"].input_ids[i]==cans_token_id).nonzero().shape[0]>0:
                ...
                for j, (idx, item_emb) in enumerate(zip(idx_tensor,cans_item_embeds[i,:batch["len_cans"][i].item()])):
                    input_embeds[i,idx] = item_emb
