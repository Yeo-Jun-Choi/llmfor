    ...
    def __call__(self, batch):
        ...
        if p > thresh_hold:
            for i, sample in enumerate(batch):
                ...
                if '[UserHere]' in input_text:
                    insert_prompt='[UserEmb]'
                    input_text=input_text.replace('[UserHere]',insert_prompt)
        ...
        if self.train:
            targets_text_=[target_text+self.terminator for target_text in targets_text]
            for idx, targets in enumerate(targets_text_):
                # Add RUHG
                targets_text_[idx] = targets+' The reversed user history is: ' + batch[idx]['reverse_seq_str']
            
            inputs_pair = [[p, t] for p, t in zip(inputs_text, targets_text_)]
            ...
            new_batch={"tokens":batch_tokens,
                       ...
                       "can_title": [sample['cans_name'] for sample in batch],
